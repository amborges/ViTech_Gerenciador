...
### com o software uavs3, modificações para outros formatos é relativamente  ###
...
#Precisa baixar o uavs3?
DOWNLOAD = False

#Se precisar que o download do uavs3 seja regredido para alguma versão passada
...
#caminho da pasta de compilação do uavs3
#é de lá que ele vai compilar e manter os executáveis
#Caso houver mais de uma versão de uavs3, ir adicionando as pastas
# >>>
...
def GENERATE_COMMAND(core, cq, folder, video_path, codec_path, home_path, path_id, extra_param, width, height, subsample, bitdepth, num_frames, frames_per_unit, unit_size):
	...
	#comando completo
	#./uavs3e/build/linux/uavs3enc --verbose 2 --signature 1 --speed_level 0 --wpp_threads 1 --frm_threads 1 --i_period 1 --frames {FPS} --width {WDT} --height {HGT} --input_bit_depth {BD} --fps_num 30 --fps_den 1 --rc_type 0 --qp 27 --input {VIDEO} -o coded_{VIDEO}.avs3
	...
	#definindo o caminho e nome do executável: ./uavs3enc
	executable_param = codec_path + CODEC_NAME
	
	#definindo o arquivo de configuração principal:
	main_cfg_param = ' --verbose 2 --signature 1 --speed_level 0 --wpp_threads 1 --frm_threads 1 --i_period 1'
	
	#Condições especiais de configuração
	#Caso vídeo for 10bit de profundidade
	#if (bitdepth == 10):
	#	main_cfg_param += ' -c ' + home_path + '/uavs3/cfg/encoder_randomaccess_main10.cfg'
	#
	#Caso vídeo tiver subamostragem 444
	#if (subsample == 444):
	#	main_cfg_param += ' -c ' + home_path + '/uavs3/cfg/encoder_randomaccess_main_rext.cfg -cf ' + str(subsample)
	
	#definindo as informações do vídeo
	video_param  = ' --width ' + str(width) #width
	video_param += ' --height ' + str(height) #hight
	video_param += ' --fps_num ' + str(frames_per_unit) + ' --fps_den ' + str(unit_size) #fps
	video_param += ' --input_bit_depth ' + str(bitdepth) #bit depth
	video_param += ' --internal_bit_depth ' + str(bitdepth) #bit depth
	
		
	#definindo o nível de quantização
	quant_param  = ' --rc_type 0 --qp ' + str(cq)
	...
	codfile_param = ' --output ' + this_folder + 'video/coded_' + path_id +  ep_name + '.avs3'
...
def get_psnr_bitrate_time(from_file):
	#abro o arquivo obtido do uavs3
	f = open(from_file)
	...
	for lst_line in f:
		...
		if foundSummary:
			...
			if(interesting_line == 2):
				psnr_line = lst_line
			if(interesting_line == 9):
				bitrate_line = lst_line
			if(interesting_line == 12):
				time_line = lst_line
				foundSummary = False
		pass
	f.close()
	...
	#  PSNR Y(dB)       : 46.5677
	#bitrate_line
	#  bitrate(kbps)    : 9516.2400
	#time_line
	#Total encoding time               = 104365.000 msec, 104.365 sec
...
#Função que permite baixar o uavs3
def DO_DOWNLOAD(codec_path):
	...
	git_command = 'git clone https://github.com/uavs3/uavs3e ' + codec_path
...
#Função que compila o uavs3e
#O código já adapta para possíveis versões diferentes de sistema operacional
def DO_COMPILE(os_version, codec_path):
	#se precisar compilar o uavs3e, então COMPILA
	if(os.path.exists(codec_path)):
		#se a pasta já existe, apagar tudo pra deixar uma compilação limpa
		os.system('rm -rf ' + codec_path)
	os.system('mkdir ' + codec_path)
	
	#É preciso modificar o software na mão para permitir que ele trabalhe com 10bits
	with open(codec_path + '../inc/com_api.h', 'r+') as f:
		text = f.read()
		text = re.sub('#define BIT_DEPTH 8', '#define BIT_DEPTH 10', text)
		f.seek(0)
		f.write(text)
		f.truncate()
